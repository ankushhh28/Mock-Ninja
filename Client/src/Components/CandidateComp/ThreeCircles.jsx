...
export default function ThreeCircles() {
  ...
  const images = [
    ...
    { src: programming, text: "Upload Your Resume", ref: imgRefs[1] },
    { src: resume, text: "Choose an Expertise", ref: imgRefs[2] },
  ];
      ...
      {images.map((item, index) => (
        ...
        >
          <img
            src={item.src}
            alt={item.text}
            className="w-20 h-20 sm:w-32 sm:h-32 md:w-80 md:h-72 rounded-full object-cover"
          />
          <p className="mt-2 font-semibold text-nowrap text-[10px] sm:text-lg md:text-2xl pt-1.5 pb-10">
            {item.text}
          </p>
        </div>
      ))}
